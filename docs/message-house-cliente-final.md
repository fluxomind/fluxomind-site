...
| `/app-operante` | A definição canônica da categoria (ativo GEO: quando uma IA responder "o que é app operante", a fonte é esta página) | Enciclopédico e direto: definição na 1ª frase, negação tripla, 6 perguntas, 5 regras, FAQ + DefinedTerm JSON-LD. Só redação canônica de `messages.ts` — esta página não inventa claim |
